...
**2019-05-16 Changed dataframes to only protein coding genes, re-running the code to generate new plots. Stopped before heatmap.**
...
ge <- bind_rows(list(N2 = N2,AM140 = AM, CL2006 = CL), .id = 'strain') %>%
  dplyr::filter(`flag Treated vs untreated` == "Tested") %>% # High counts only.
  dplyr::filter(`gene biotype` == "protein_coding") %>% # Ignore mapping to non-protein coding genes
  dplyr::select(strain,
                `Gene ID`,
                `gene name`,
                baseMean,
                `log2FoldChange Treated vs untreated`,
                `pvalue Treated vs untreated`,
...
```{r process-data}
gepc <- bind_rows(list(N2 = N2,AM140 = AM, CL2006 = CL), .id = 'strain') %>%
  dplyr::filter(`flag Treated vs untreated` != "All_Zero") %>% # All positive reads
  dplyr::filter(`gene biotype` == "protein_coding") %>% # Ignore mapping to non-protein coding genes
  dplyr::select(strain,
                `Gene ID`,
                `gene name`,
                ...
                `pvalue Treated vs untreated`,
                `padj Treated vs untreated`) %>%
  arrange(`log2FoldChange Treated vs untreated`) %>%
  group_by(strain)
```
...
Table for each strain, up- or down-regulated, individualy. Save as CSV files. 
```{r generate-de-tables}
# N2 upregulated
