...
# Heatmap: Oppositely regulated genes
...
fc <- 
  gepc_tested %>% 
  dplyr::select(-baseMean,-pvalue,-flag,-FDR_adj_pvalue) %>%
  spread(key = strain,
         value = c(log2FC)) %>%
  dplyr::mutate(diff = abs(AM140-CL2006)) %>% # difference between CL2006 and AM140 FC
  dplyr::arrange(desc(diff))
```

**Heatmap by FC, can also try by Z-score**

```{r fc-matrix}
mat <- cbind(fc$N2,fc$AM140,fc$CL2006)
rownames(mat) <- fc$Gene_Name
colnames(mat) <- c('N2','AM140','CL2006')
