...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("..")) # Root Project Directory
```

Set parameters
```{r parameters, results='hide'}
today <- Sys.Date()
path <- list(results = "results/",
            data = "data/",
            raw = "raw/")
```
...
# Tidyverse, viridis, bioconductor, gplots, RColorBrewer
conda install -c r r-tidyverse r-viridis r-gplots r-rcolorbrewer

# Bioconductor packages
conda install -c bioconda r-venndiagram bioconductor-biomart \
bioconductor-org.ce.eg.db bioconductor-fgsea
```
...
Import the gene expression data sent by the Technion.
...
## Load Gene Lists
Vector of both up- and down-regulated genes (*p*-value < 0.1).
```{r load-data-genelist}
load("data/up.Rdata")
load("data/dn.Rdata")
```
