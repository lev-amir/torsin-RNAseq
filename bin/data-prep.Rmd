...
```{r plot}
# dev.off()
svg(filename = "results/2019-05-20-response-AM140-CL2006.svg",
          width = 3, 
          height = 4,
          pointsize = 3)


fc[1:50,] %>% # Selected top 50 genes
  ggplot( 
  aes(x = reorder(Gene_Name, -diff), # Order by difference size
      y = diff,
      fill = sign)) + # Color by direction
  geom_col() + 
  #ylab(
  #  expression('|'~(log[2]~FC~AM140)-(log[2]~FC~CL2006)~'|')
  #  ) + 
  ylab('Response Difference') + # See above note for how calculated
  xlab('') + 
  labs(fill = "Torsin RNAi effect") + # Legend title
  ...
  theme(legend.position = c(.8,.9),
        text = element_text(size=7)) +  # font size of entire plot
  guides(fill=guide_legend(keywidth=0.1, # Space between legend items
                           keyheight=0.1,
                           default.unit="inch")) + 
  scale_fill_grey(start = 0.2, end = 0.8) # Colors used
...
```{r de-rename}
# Change ge to gepc_tested or gepc_read
tab <- ge %>% 
  filter(`Gene ID` %in% goi) %>% 
  dplyr::rename(Gene_Name = `gene name`,
                Gene_ID = `Gene ID`,
                log2FC = `log2FoldChange Treated vs untreated`,
                FDR_p = FDR_adj_pvalue) %>%
...
```{r}
write(ge %>% filter(FDR_adj_pvalue < 0.1) %>% pull(`Gene ID`) %>% unique(),
      file = "a.txt")
```
